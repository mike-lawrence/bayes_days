...
#look at the real-valued sigmas
samples %>% 
	dplyr::group_by(Iteration,Chain) %>%
	plyr::summarize(
		center = value[Parameter=='log_sigma_center']
		, diff = value[Parameter=='log_sigma_diff']
	)  %>%
	dplyr::ungroup() %>%
	plyr::mutate(
		cell1 = exp(center+diff/2)
		, cell2 = exp(center-diff/2)
		, diff = cell1-cell2
	) %>%
	plyr::summarize(
		cell1 = quantile(cell1,probs=c(.025,.5,.975))
		, cell2 = quantile(cell2,probs=c(.025,.5,.975))
		, diff = quantile(diff,probs=c(.025,.5,.975))
	)
